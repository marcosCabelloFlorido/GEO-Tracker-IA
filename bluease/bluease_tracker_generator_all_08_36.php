...
— NO decir que somos ONG o sin ánimo de lucro"
  ...
  <script>
    ...
    const SCREENS = { 1: 'Configuración', 2: 'Análisis', 3: 'Dashboard', 4: 'Plan de acción', 5: 'Generador Contenido', 6: 'Bandeja de aprobación', 7: 'Historial de Temas', 8: 'Biblioteca' };
    ...
    let S = {
      ...
      contenido: null,
      propuestas: {
        config: { tamanoLote: 2, frecuenciaDias: 7, ultimaGeneracion: null },
        cola: []
      },
      historialTemas: [],
      historialPlanes: []
        ...
        if (v.propuestas) {
          ...
          if (S.propuestas.cola) {
            S.propuestas.cola.forEach(function (p) {
              if (p.estado === 'generando') { p.estado = 'pendiente'; p.errorMotor = null; }
            });
          }
        }
    ...
    const ANTHROPIC_API_KEY = 'PONER LA KEY DE ANTROPIC AQUI'; // REEMPLAZAR POR TU CLAVE REAL
    ...
    function renderAnalysis(idx) {
      ...
      let totalFields = 8;
      if (sc.error || sc.isSPA) { baseFields.push('blog', 'faq', 'headings', 'casos', 'glosario', 'alttext'); totalFields = 14; }
      baseFields.forEach(f => { const raw = S.analysis.metrics[idx + '_' + f]; if (raw && raw !== 'nosé') filled++; });
    ...
    function updateFechaManual(el) { S.analysis.metrics[S.activeComp + '_ultimoPost'] = el.value.trim(); save(); }
    function updatePostsAnoManual(el) { const v = parseInt(el.value); S.analysis.metrics[S.activeComp + '_postsAno'] = isNaN(v) ? 0 : v; save(); }
    ...
    function startAnalysis() {
      ...
      save(); goScreen(2); renderTabs(); runAnalysis();
    }
    ...
    function construirPrompt() {
      ...
      const metricas = [
        { id: 'chatgpt', label: 'Aparece en ChatGPT', pts: 15 },
        { id: 'perplexity', label: 'Aparece en Perplexity', pts: 15 },
        { id: 'googlesge', label: 'Aparece en Google SGE', pts: 10 },
        { id: 'faq', label: 'Tiene FAQ estructurada', pts: 10 },
        { id: 'headings', label: 'Usa encabezados H1/H2', pts: 8 },
        { id: 'casos', label: 'Tiene casos de uso/éxito', pts: 8 },
        { id: 'comparativos', label: 'Artículos comparativos', pts: 7 },
        { id: 'blog', label: 'Contenido actualizado', pts: 7 },
        { id: 'fuentes', label: 'Cita fuentes y datos', pts: 5 },
        { id: 'glosario', label: 'Glosario o definiciones', pts: 5 },
        { id: 'alttext', label: 'Alt text en imágenes', pts: 5 },
        { id: 'schema', label: 'Schema Markup', pts: 5 },
      ];
      ...
      metricas.forEach(m => {
        ...
        if (maxComp > miPts) brechas.push({ label: m.label, gap: maxComp - miPts, max: m.pts });
      });
    ...
    async function generarPlanAccion() {
      ...
      try {
        ...
        var ahora = new Date();
        var fechaLabel = ahora.toLocaleDateString('es-ES', { day: '2-digit', month: '2-digit', year: 'numeric' })
          + ' ' + ahora.toLocaleTimeString('es-ES', { hour: '2-digit', minute: '2-digit' });
        S.historialPlanes.unshift({ id: ahora.getTime(), fechaLabel: fechaLabel, plan: plan, notionChecks: {} });
        ...
        var accionesContenido = (plan.acciones || []).filter(function (a) { return a.requiere_articulo === true; });
        if (accionesContenido.length > 0) {
          ...
          p.cola = nuevasPropuestas.concat(p.cola.filter(function (x) { return x.origen !== 'plan'; }));
    ...
    function renderPlan(plan, historialIdx) {
      ...
      var colImpacto = {
        alto: { bg: 'rgba(239,68,68,.12)', color: '#ef4444' },
        medio: { bg: 'rgba(245,158,11,.12)', color: '#b45309' },
        bajo: { bg: 'rgba(34,197,94,.12)', color: '#16a34a' }
      };
      var textoPlazo = { inmediato: '⚡ Inmediato', corto: '📅 Corto plazo', medio: '🗓 Medio plazo' };
      ...
      var accionesContenido = acciones.filter(function (a) { return a.requiere_articulo === true; });
      var accionesManuales = acciones.filter(function (a) { return a.requiere_articulo !== true; });
    ...
    function getPropuestas() {
      if (!S.propuestas) S.propuestas = { config: { tamanoLote: 2, frecuenciaDias: 7, ultimaGeneracion: null }, cola: [] };
      if (!S.propuestas.config) S.propuestas.config = { tamanoLote: 2, frecuenciaDias: 7, ultimaGeneracion: null };
    ...
    function actualizarLabelProximo() {
      ...
      if (!el) return;
      if (!p.config.ultimaGeneracion) { el.textContent = 'Sin lote anterior'; return; }
      var proximo = new Date(p.config.ultimaGeneracion + p.config.frecuenciaDias * 86400000);
      el.textContent = 'Próximo lote: ' + proximo.toLocaleDateString('es-ES', { day: '2-digit', month: '2-digit', year: 'numeric' });
    }
    ...
    function actualizarStatusLabel() {
      ...
      var pendientes = p.cola.filter(function (x) { return x.estado === 'pendiente'; }).length;
      var completados = p.cola.filter(function (x) { return x.estado === 'completado'; }).length;
      var fecha = new Date(p.config.ultimaGeneracion).toLocaleDateString('es-ES', { day: '2-digit', month: 'short', year: 'numeric' });
    ...
    async function generarLote() {
      ...
      if (!planActivo || !planActivo.acciones) {
        ...
        if (btn) { btn.disabled = false; btn.textContent = '📦 Generar lote'; }
        return;
      }

      var tareasContenido = planActivo.acciones.filter(function (a) { return a.requiere_articulo === true; });
      ...
      try {
        var response = await fetch(MY_WORKER + '/api/plan', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ prompt: promptLote })
        });
        ...
        resultado.propuestas.forEach(function (prop) {
          S.historialTemas.push({ titulo: prop.titulo, fecha: Date.now() });
        });
    ...
    function renderColaPropostas() {
      ...
      var ESTADO_BADGE = {
        pendiente: { bg: 'rgba(100,116,139,.12)', color: 'var(--text-muted)', txt: '⏳ Pendiente' },
        generando: { bg: 'rgba(37,99,235,.12)', color: 'var(--primary)', txt: '⚙️ Generando…' },
        error: { bg: 'rgba(239,68,68,.12)', color: '#dc2626', txt: '❌ Error' }
      };
    ...
    function activarPropuesta(idx) {
      ...
      if (tipoSel) {
        for (var i = 0; i < tipoSel.options.length; i++) {
          if (tipoSel.options[i].value === prop.tipo) { tipoSel.selectedIndex = i; break; }
        }
      }
      ...
      _brechaActiva = { id: prop.brechaId, label: prop.brechaLabel, gap: prop.gap };
    ...
    function seleccionarBrecha(id, label, gap, tipoSugerido, temaSugerido, kwsSugeridas) {
      _brechaActiva = { id: id, label: label, gap: gap };

      var tipoSel = document.getElementById('gen-tipo');
      if (tipoSel && tipoSugerido) {
        for (var i = 0; i < tipoSel.options.length; i++) {
          if (tipoSel.options[i].value === tipoSugerido) { tipoSel.selectedIndex = i; break; }
        }
      }
      var temaEl = document.getElementById('gen-tema');
      var kwsEl = document.getElementById('gen-kws');
    ...
    function _getGenParams() {
      ...
      if (!tema) { toast('⚠️', 'Por favor, introduce un tema principal.', 'warn'); return null; }
      ...
      return { tipo, tema, kws, tono, comp, empresaName, sector, linkLinkedIn, linkInstagram, contextBrecha, slugArticulo };
    }
    ...
    function _iniciarLoading(label) {
      ...
      progressFill.style.width = '0%'; progressLbl.textContent = '0%';
      let pct = 0;
      const intv = setInterval(function () {
        if (pct < 92) { pct += Math.random() * 4 + 1; if (pct > 92) pct = 92; }
        else if (pct < 98) { pct += 0.12; }
        progressFill.style.width = Math.min(pct, 98) + '%';
        progressLbl.textContent = Math.round(Math.min(pct, 98)) + '%';
      }, 500);
      return { loadingDiv, progressFill, progressLbl, intv };
    }

    // ── Función auxiliar: finalizar con éxito ──
    function _finalizarLoading(loadingDiv, progressFill, progressLbl, intv) {
      clearInterval(intv);
      progressFill.style.width = '100%'; progressLbl.textContent = '100%';
      ...
      if (_propMotorId !== null && S.propuestas && S.propuestas.cola) {
        var p = getPropuestas();
        var motorIdx = p.cola.findIndex(function (x) { return x.id === _propMotorId; });
        if (motorIdx >= 0) {
      ...
      setTimeout(function () {
        ...
        if (barra) {
          ...
          if (btnAp) { btnAp.disabled = false; btnAp.style.opacity = '1'; }
          if (btnRe) { btnRe.disabled = false; btnRe.style.opacity = '1'; }
        }
    ...
    function cargarBDyRest() {
      ...
      if (taBd)   { taBd.value   = bd;   document.getElementById('bd-chars').textContent   = bd.length   + ' caracteres'; }
      if (taRest) { taRest.value = rest;  document.getElementById('rest-chars').textContent = rest.length + ' caracteres'; }
    }
    ...
    function detectarEstructura(markdown) {
      ...
      if (tieneNumerada && tieneViñetas < 5 && !tieneTabla) return { num: 5, nombre: 'Lista + acción', color: '#8b5cf6' };
      // E2: texto narrativo con cita destacada, sin tabla ni viñetas
      if (tieneCita && tieneViñetas < 4 && !tieneTabla) return { num: 2, nombre: 'Narrativa + autoridad', color: '#0ea5e9' };
      // E3: muchas viñetas, sin TL;DR, tabla pequeña o sin tabla
      if (tieneViñetas >= 10 && !tieneTldr && tieneH2 >= 3) return { num: 3, nombre: 'Escaneable + visual', color: '#10b981' };
      // E4: párrafos largos, pocos elementos visuales
      if (maxParrafo > 150 && tieneViñetas < 6 && !tieneTabla) return { num: 4, nombre: 'Profundidad + reflexión', color: '#f59e0b' };
      // E1: tabla + viñetas + párrafos medianos (default para datos)
      return { num: 1, nombre: 'Datos + guía accionable', color: '#007AEB' };
    }
    ...
    function validarCalidadBlog(markdown, tipo) {
      ...
      var rangos = {
        'faq avanzada':          { min: 600,  max: 900,  parrafoMax: 120 },
        'glosario tecnico':      { min: 500,  max: 800,  parrafoMax: 80  },
        'guia operativa':        { min: 1200, max: 1800, parrafoMax: 150 },
        'caso de exito':         { min: 1500, max: 2200, parrafoMax: 180 },
        'comparativo analitico': { min: 1400, max: 2000, parrafoMax: 180 }
      };
      var tipoNorm    = (tipo || '').toLowerCase().trim();
      var rangoActual = rangos[tipoNorm] || { min: 800, max: 1800, parrafoMax: 180 };
      ...
      var h2Preguntas = h2s.filter(function(h) { return /¿|\?/.test(h); }).length;
      ...
      if (tldrMatch) {
        ...
        // Eliminar duplicados
        var cifrasUnicas = todasCifras.filter(function(v, i, a) { return a.indexOf(v) === i; });
    ...
    function accionarDesdeGenerador(estado) {
      if (!S.contenido || !S.contenido.generado) {
        toast('⚠️', 'No hay contenido generado para aprobar', 'warn'); return;
      }
      ...
      var btnAp = document.getElementById('btn-aprobar-gen');
      var btnRe = document.getElementById('btn-rechazar-gen');
      if (btnAp) { btnAp.disabled = true; btnAp.style.opacity = '.5'; }
      if (btnRe) { btnRe.disabled = true; btnRe.style.opacity = '.5'; }
    ...
    async function generarBlog() {
      const params = _getGenParams(); if (!params) return;
      const { tipo: tipoRaw, tema, kws, tono, comp, empresaName, sector, contextBrecha } = params;
      const tipo = (tipoRaw || '').toLowerCase().trim();
      const { loadingDiv, progressFill, progressLbl, intv } = _iniciarLoading('📝 Generando artículo...');
...
Responde SOLO con el artículo en Markdown. Sin JSON ni etiquetas extra.`;
      ...
      try {
        while (intentos < maxIntentos) {
          ...
          if (intentos > 1) {
            ...
            promptActual = promptBlog +
              ...
              validacion.errores.map(function(e) { return '- ' + e; }).join('\n');
          }
        ...
        if (!validacion.ok) {
          ...
          var rangosLocal = {
            'faq avanzada': 900, 'glosario tecnico': 800, 'guia operativa': 1800,
            'caso de exito': 2200, 'comparativo analitico': 2000
          };
        ...
        if (!S.historialTemas.some(t => t.titulo.toLowerCase() === tema.toLowerCase())) {
          S.historialTemas.push({ titulo: tema, fecha: Date.now() });
        }
    ...
    async function generarRRSS() {
      ...
      const params = _getGenParams(); if (!params) return;
      const { empresaName, sector, linkLinkedIn, linkInstagram, tema } = params;
      const { loadingDiv, progressFill, progressLbl, intv } = _iniciarLoading('📱 Generando posts para RRSS...');
...
[INSTAGRAM_END]`;
      ...
      try {
        ...
        function extraerSeccion(etiqueta, txt) {
          ...
          const m = txt.match(re); return (m && m[1] && m[1].trim()) ? m[1].trim() : '';
        }
...
[LINKEDIN_END]`;
      ...
      try {
        const response = await fetch(MY_WORKER + '/api/plan', {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ prompt: promptGEO }),
        ...
        if (_propuestaEnGeneracionIdx >= 0 && S.propuestas && S.propuestas.cola[_propuestaEnGeneracionIdx]) {
          ...
          if (pIdx >= 0) {
            S.propuestas.cola[pIdx].contenido = {
              blog: contenidoBlog, linkedin: contenidoLinkedin, instagram: contenidoInstagram, generado: true
            };
            S.propuestas.cola[pIdx].estado = 'completado';
          }
        }
        ...
        if (temaGen) {
          ...
          if (!yaExiste) {
            S.historialTemas.push({ titulo: temaGen, fecha: Date.now() });
            save();
          }
        }
    ...
    function copiarUTMModal(texto, btnId) {
      if (!texto) { toast('⚠️', 'No hay UTM para copiar', 'warn'); return; }
    ...
    function copiarUTM(red) {
      ...
      if (!utm) { toast('⚠️', 'No hay UTM generado', 'warn'); return; }
      navigator.clipboard.writeText(utm).then(function () {
        var btn = document.getElementById('utm-btn-' + red);
        if (btn) { btn.textContent = '✓ Copiado'; btn.style.background = '#10b981'; setTimeout(function () { btn.textContent = '📋 Copiar'; btn.style.background = 'var(--primary)'; }, 2000); }
        toast('📋', 'UTM de ' + (red === 'linkedin' ? 'LinkedIn' : 'Instagram') + ' copiado');
      });
    }

    function copiarSeccion(tipo) {
      if (!S.contenido) { toast('⚠️', 'No hay contenido generado', 'warn'); return; }
      var texto = '';
      ...
      if (!texto) { toast('⚠️', 'No hay contenido para copiar', 'warn'); return; }
   ...
   ───────────────────────────────────────────────────────── */
    ...
    function cambiarEstadoBandeja(idx, nuevoEstado) {
      ...
      if (nuevoEstado === 'descartado' && prop.tema) {
        ...
        if (!S.historialTemas.some(function (t) {
          ...
          S.historialTemas.push({ titulo: prop.tema, fecha: Date.now() });
        }
      }
    ...
    function renderBandeja(filtro) {
      ...
      var filtradas = filtro === 'todos'
        ? todas
        : todas.filter(function (p) { return p.estado === filtro; });
      ...
      var pendientes = todas.filter(function (p) { return p.estado === 'pendiente'; }).length;
      ...
      if (barra && label && texto) {
        ...
        if (pct < 50) {
          color = '#10b981'; bgLabel = 'rgba(16,185,129,.12)'; colorLabel = '#059669'; nivel = '🟢 Ligera';
        } else if (pct < 70) {
          color = '#f59e0b'; bgLabel = 'rgba(245,158,11,.12)'; colorLabel = '#b45309'; nivel = '🟡 Moderada';
        } else {
          color = '#ef4444'; bgLabel = 'rgba(239,68,68,.12)'; colorLabel = '#dc2626'; nivel = '🔴 Alta';
        }
      ...
      var TIPO_LABEL = {
        'faq avanzada': { icon: '❓', label: 'FAQ' },
        'comparativo analitico': { icon: '⚖️', label: 'Comparativo' },
        'caso de exito': { icon: '💼', label: 'Caso de éxito' },
        'glosario tecnico': { icon: '📖', label: 'Glosario' },
        'guia operativa': { icon: '📋', label: 'Guía' }
      };

      var ESTADO_CONFIG = {
        pendiente: { bg: 'rgba(245,158,11,.1)', color: '#b45309', label: '✍️ Por generar' },
        aprobado: { bg: 'rgba(16,185,129,.1)', color: '#059669', label: '✅ Aprobada' },
        edicion: { bg: 'rgba(37,99,235,.1)', color: '#1d4ed8', label: '🔖 Por aprobar' },
        descartado: { bg: 'rgba(100,116,139,.1)', color: '#475569', label: '🗑️ Descartada' },
        completado: { bg: 'rgba(16,185,129,.1)', color: '#059669', label: '✓ Completada' }
      };
      ...
      filtradas.forEach(function (prop) {
        ...
        var tInfo = TIPO_LABEL[prop.tipo] || { icon: '📝', label: 'Blog' };
        var eConfig = ESTADO_CONFIG[prop.estado] || ESTADO_CONFIG.pendiente;
        var fecha = new Date(prop.fechaCreacion).toLocaleDateString('es-ES', { day: '2-digit', month: '2-digit', year: 'numeric' });
    ...
    function confirmarRegeneracion(idx) {
      ...
      _brechaActiva = { id: prop.brechaId || 'regenerar', label: prop.brechaLabel, gap: prop.gap || 0 };
    ...
    async function cambiarTemaPropuesta(idx) {
      ...
      try {
        ...
        _brechaActiva = { id: prop.brechaId || 'cambio_tema', label: prop.brechaLabel, gap: prop.gap || 0 };
    ...
    function abrirDetallePropuesta(idx) {
      ...
      modal.addEventListener('click', function (e) { if (e.target === modal) cerrarDetalle(); });
    ...
    function renderTabDetalle(tab, c, idx) {
      ...
      if (tab === 'imagen') {
        ...
        if (!promptBlog && !promptInsta) {
          ...
          if (legacyM) { promptBlog = legacyM[1].trim(); promptInsta = legacyM[1].trim(); }
        }
    ...
    function copiarCampoImg(editorId, campo, idx) {
      ...
      if (!el || !el.value.trim()) { toast('⚠️', 'No hay contenido para copiar', 'warn'); return; }
    ...
    function toggleVistaEditor(idx) {
      ...
      if (modoMd) {
        ed.style.display = 'block'; md.style.display = 'none';
        if (tb) tb.style.display = 'flex';
        if (btn) btn.textContent = '📄 Ver Markdown';
      } else {
        var prop = S.propuestas.cola[idx];
        md.value = (prop && prop.contenido && prop.contenido.blog) || '';
        ed.style.display = 'none'; md.style.display = 'block';
    ...
    function renderHistorialTemas() {
      ...
      temasOrdenados.forEach(function (t) {
        const d = new Date(t.fecha).toLocaleDateString('es-ES', { day: '2-digit', month: '2-digit', year: 'numeric', hour: '2-digit', minute: '2-digit' });
    ...
    function eliminarTemaHistorial(timestamp) {
      ...
      S.historialTemas = S.historialTemas.filter(function (t) { return t.fecha !== timestamp; });
      save();
      renderHistorialTemas();
    ...
    function exportarMarkdown(idx) {
      ...
      if (!prop || !prop.contenido || !prop.contenido.blog) {
        toast('⚠️', 'No hay contenido de blog para exportar', 'warn'); return;
      }
      var blob = new Blob([prop.contenido.blog], { type: 'text/markdown;charset=utf-8' });
    ...
    function renderBiblioteca(filtro) {
      ...
      filtradas.forEach(function (prop) {
        ...
        var fechaAp = prop.fechaEstado
          ? new Date(prop.fechaEstado).toLocaleDateString('es-ES', { day: '2-digit', month: '2-digit', year: 'numeric' })
          : '—';
        var fechaPub = prop.fechaPublicacion
          ? new Date(prop.fechaPublicacion).toLocaleDateString('es-ES', { day: '2-digit', month: '2-digit', year: 'numeric' })
          : null;
